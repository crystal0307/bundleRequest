                        ...
                        if(events != null && events != undefined && events.length > 0) {
                            _.forEach(events,(event) => {
                                ...
                                if (isDo && isDo.length <= 0) {
                                    UserModel.findOneAndUpdate({'openId': data.openId}, {$push: {'reminders': event.id}}, (err, res) => {
                                        if(!err)
                                        {
...
function sendTem(uId,event){
    WUser.find({unionId:uId}).then(data => {
        if(data){
                var datas = {
                    "first":{"value":"您好!您的在办项目有最新进度,请及时跟进办理:"},
                    "keyword1":{"value":event.title},
                    "keyword2":{"value":"孕前检查"},
                    "keyword3":{"value":(new moment()).format("YYYY年MM月DD日")},
                    "remark":{"value":"点击详情可跟进办理"}
                }
